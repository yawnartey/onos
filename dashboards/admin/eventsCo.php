<?php 
	if(isset($_POST["action"])){
		...
		if($_POST["action"] == "fetch"){
			$query = "SELECT * FROM eventsco ORDER BY id DESC";
			...
			while($row = mysqli_fetch_array($result)){
				...
				<div class="media wow fadeIn data-wow-delay="0.8">
					<div class="row">
						<div class="col-md-5">
							<img src="data:image/jpeg;base64, '.base64_encode($row['image']).'" class="img-responsive" style="height:35%; width:100%; "/>
		...
		if($_POST["action"] == "insert"){
			...
			$query = "INSERT INTO eventsco(image, image_title, post_title, post_content) VALUES ('$file', '$image_title', '$post_title', '$post_content')"; 
		...
		if($_POST["action"] == "update"){
			...
			$post_title = $_POST["post_title"];
			$post_content = $_POST["post_content"];
			$query = "UPDATE eventsco SET image='$file', image_title='$image_title', post_title='$post_title', post_content='$post_content' WHERE id='".$_POST["image_id"]."' "; 
			if(mysqli_query($connect, $query)){
		...
		if($_POST["action"]=="delete"){
			$query = "DELETE FROM eventsco WHERE id='".$_POST["image_id"]."' "; 
			if(mysqli_query($connect, $query)){
