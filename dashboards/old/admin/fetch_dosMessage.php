<?php
	...
	<div class="media wow fadeIn" data-wow-delay="0.2s">
	 ...
	 $output .= get_dosPost($connect, $row["dosPost_id"]);
	}

	echo $output;

	function get_dosPost($connect, $parent_id = 0, $marginleft = 0)
	{
		$query = "
		SELECT * FROM dos_messages WHERE parent_dosPost_id = '".$parent_id."'";
		$output = '';
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		  ...
		  foreach($result as $row)
		{
		   		...
		   		<div>
				    ...
				    <p>'.$row["dosPost"].'</p><br>
				   	...
				   	<div class="" align="right">
				    	<button type="button" class="btn btn-light-blue reply btn-sm" id="'.$row["dosPost_id"].'">
		   ...
		   ';
		   $output .= get_dosPost($connect, $row["dosPost_id"], $marginleft);
		  }
		 }
		return $output;
	}
