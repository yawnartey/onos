<?php
	...
	// events
	$query = "SELECT * FROM eventsCo WHERE parent_eventsCoPost_id = '0' ORDER BY eventsCoPost_id DESC ";
	...
	<div class="media wow fadeIn" data-wow-delay="0.2s">
	 	<div class="media-body ml-4">
		  	<h4 class="media-heading font-bold dark-grey-text">
		  		<b>'.$row["eventsCoPost_sender_name"].'</b>
	 ...
	 $output .= get_eventsCoPost($connect, $row["eventsCoPost_id"]);
	}

	echo $output;

	function get_eventsCoPost($connect, $parent_id = 0, $marginleft = 0)
	{
		$query = "
		SELECT * FROM eventsCo WHERE parent_eventsCoPost_id = '".$parent_id."'";
		$output = '';
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		  ...
		  foreach($result as $row)
		{
		   ...
		   <div class="media wow fadeIn" data-wow-delay="0.2s" style="margin-left:'.$marginleft.'px">
		   		<div>
				    ...
				    <p>'.$row["eventsCoPost"].'</p><br>

				   	<div class="" align="right">
				    	<button type="button" class="btn btn-light-blue reply btn-sm" id="'.$row["eventsCoPost_id"].'">
		   ...
		   ';
		   $output .= get_eventsCoPost($connect, $row["eventsCoPost_id"], $marginleft);
		  }
		 }
		return $output;
	}
