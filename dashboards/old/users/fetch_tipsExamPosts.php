<?php
	...
	// events
	$query = "SELECT * FROM tipsExam WHERE parent_tipsExamPost_id = '0' ORDER BY tipsExamPost_id DESC ";
	...
	<div class="media wow fadeIn" data-wow-delay="0.2s">
	 	<div class="media-body ml-4">
		  	<h4 class="media-heading font-bold dark-grey-text">
		  		<b>'.$row["tipsExamPost_sender_name"].'</b>
	 ...
	 $output .= get_tipsExamPost($connect, $row["tipsExamPost_id"]);
	}

	echo $output;

	function get_tipsExamPost($connect, $parent_id = 0, $marginleft = 0)
	{
		$query = "
		SELECT * FROM tipsExam WHERE parent_tipsExamPost_id = '".$parent_id."'";
		$output = '';
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		  ...
		  foreach($result as $row)
		{
		   ...
		   <div class="media wow fadeIn" data-wow-delay="0.2s" style="margin-left:'.$marginleft.'px">
		   		<div>
				    ...
				    <p>'.$row["tipsExamPost"].'</p><br>

				   	<div class="" align="right">
				    	<button type="button" class="btn btn-light-blue reply btn-sm" id="'.$row["tipsExamPost_id"].'">
		   ...
		   ';
		   $output .= get_tipsExamPost($connect, $row["tipsExamPost_id"], $marginleft);
		  }
		 }
		return $output;
	}
