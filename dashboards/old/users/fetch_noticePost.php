	...
	<div class="media wow fadeIn" data-wow-delay="0.2s">
	 	<div class="media-body ml-4">
		  	<h4 class="media-heading font-bold dark-grey-text">
		  		<b>'.$row["noticePost_sender_name"].'</b>
	 ...
	 $output .= get_noticePost($connect, $row["noticePost_id"]);
	}

	echo $output;

	function get_noticePost($connect, $parent_id = 0, $marginleft = 0)
	{
		$query = "
		SELECT * FROM notice WHERE parent_noticePost_id = '".$parent_id."'";
		$output = '';
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		  ...
		  foreach($result as $row)
		{
		   ...
		   <div class="media wow fadeIn" data-wow-delay="0.2s" style="margin-left:'.$marginleft.'px">
		   		<div>
				    ...
				    <p>'.$row["noticePost"].'</p><br>

				   	<div class="" align="right">
				    	<button type="button" class="btn btn-light-blue reply btn-sm" id="'.$row["noticePost_id"].'">
		   ...
		   ';
		   $output .= get_noticePost($connect, $row["noticePost_id"], $marginleft);
		  }
		 }
		return $output;
	}
